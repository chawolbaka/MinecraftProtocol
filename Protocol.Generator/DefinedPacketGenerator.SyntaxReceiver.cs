                ...
                if (context.Node is FieldDeclarationSyntax fieldDeclarationSyntax && fieldDeclarationSyntax.AttributeLists.Count > 0)
                {
                    foreach (VariableDeclaratorSyntax variable in fieldDeclarationSyntax.Declaration.Variables)
                    {
                        ...
                        if (attributeData != null)
                        {
                            ...
                            if (attributeData.ConstructorArguments.Length >= 6)
                                ap.IsOptional = Convert.ToBoolean(attributeData.ConstructorArguments[5].Value);
                            ...
                            var ov = attributeData.NamedArguments.SingleOrDefault(x => x.Key == nameof(AttributeProperty.IsOverrideProperty)).Value;
                            var op = attributeData.NamedArguments.SingleOrDefault(x => x.Key == nameof(AttributeProperty.IsOptional)).Value;
                            ...
                            if (!ov.IsNull)
                                ap.IsOverrideProperty = Convert.ToBoolean(ov.Value);
                            if (!op.IsNull)
                                ap.IsOptional = Convert.ToBoolean(op.Value);
